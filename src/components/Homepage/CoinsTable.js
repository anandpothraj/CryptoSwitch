...
const CoinsTable = () => {
    ...
    return (
        <div style={{ textAlign : "center" , backgroundColor:"#212529"}}>
            <h2 className='py-2 text-light'>Cryptocurrency Prizes by Market Cap</h2>
            <Form.Control type="text" className='w-75 m-auto' placeholder="Search For A Crypto Currency..." value={search} onChange={(e)=>setSearch(e.target.value)}/>
            <div className='tableContainer'>
                {
                    loading ? <ProgressBar variant="warning" className='w-75 m-auto my-2' striped animated now={progress} />
                        :
                        <Table className='w-75 m-auto mt-2'>
                                ...
                                handleSearch().slice(( currentPage - 1 ) * 10, ( currentPage - 1 ) * 10 + 10 ).map((row) => {
                                    ...
                                    return(
                                        <tr key={row.name} onClick={ () => navigate(`/coins/${row.id}`)} style={{cursor:"pointer"}} >
                                            ...
                                            <td>{symbol}{" "}{numberWithCommas(row.current_price.toFixed(2))}</td>
                                            <td style={{color: profit > 0 ? "rgb(14, 203, 129)" : "red",fontWeight: 500}}>{profit && "+"}{row.price_change_percentage_24h.toFixed(2)}%</td>
